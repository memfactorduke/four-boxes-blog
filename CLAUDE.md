...
6. **Voice + tone + scope + citation + length CHECK pass** (Sonnet agent, **REQUIRED — never skip, even for one article**): grep the body for third-person Mark/host/video references; scan for tone violations (see the Tone Hard Rule) — casual/chatty filler, slang/internet-speak, emoji, snark, multiple exclamation points, rhetorical-question pileups, hedging mush — and **FAIL with line references if the piece does not sustain a serious, scholarly, authoritative, quotable register**; verify every named case/person/bill/topic is in the transcript OR is verified factual context tied to a transcript mention; verify every cited justice/judge against the research sheet; **count body words and FAIL if ≥ 1,000**. Report line-referenced flags; do NOT silently edit. The main thread fixes flags before image generation.
...
### Tone — serious, scholarly, authoritative, quotable (NON-NEGOTIABLE)
The body MUST read as serious, scholarly, authoritative, **journalistic** prose — the register of a constitutional law professor writing a bylined op-ed for a national paper, the kind of piece other writers and reporters will **quote**. This has been slipping; enforce it deliberately. The target is **quotable authority**: clean declarative sentences that state a position and back it with law, so any single paragraph could be lifted and cited on its own. **Encouraged:** measured argument, precise legal analysis, command of the record, principled indignation, the occasional pointed turn of phrase Mark is known for. **Forbidden in the body:** casual/chatty filler ("look,", "folks,", "let's be honest,", "here's the thing,", "I mean,"); social-media slang/internet-speak ("insane," "wild," "crazy," "nuts," "dunk," "based," "cope," "ratio," emoji); snark that reads as a tweet rather than an essay; exclamation points (at most one in the whole piece, only if truly warranted); rhetorical-question pileups; jokey asides; and hedging mush ("kind of," "sort of," "I guess") that undercuts authority. Strong opinion is welcome — express it in confident, scholarly, declarative prose. Test each sentence: would it survive in a law-review essay or serious op-ed, AND would a reporter quote it verbatim? If not, rewrite it.
