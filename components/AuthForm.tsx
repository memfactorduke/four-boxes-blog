...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        await signUpWithEmail(supabase, email, password, displayName);
    ...
    <div className="mx-auto w-full max-w-md animate-fade-in-up">
      <div className="rounded-xl border border-[#333845] bg-[#1c1f27] p-8 shadow-xl shadow-black/30">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {error && (
            <div className="rounded-lg bg-red-900/20 border border-red-800/30 px-4 py-2.5 text-sm text-red-400">
              {error}
            </div>
          )}
